...
function loadData() {
    try {
        ...
        if (savedData) {
            ...
            // Check if we need to migrate old unit to packaging
            const migrationDone = localStorage.getItem('unitToPackagingMigration');
            if (!migrationDone) {
                let updated = false;
                inventory.forEach(item => {
                    // If item doesn't have packaging field, migrate from old unit
                    if (!item.packaging && item.unit) {
                        // Check if old unit is a packaging type
                        const packagingTypes = ['Karung', 'Sak', 'Botol', 'Pack', 'Box', 'Dus', 'Pcs', 'Jerigen', 'Galon'];
                        const unitTypes = ['Kg', 'Gram', 'Liter', 'Mililiter', 'Meter', 'Cm'];
                        
                        if (packagingTypes.includes(item.unit)) {
                            item.packaging = item.unit;
                            item.unit = 'Mililiter'; // Default satuan baru
                        } else if (unitTypes.includes(item.unit)) {
                            item.packaging = 'Botol'; // Default bungkusan
                            // unit stays the same
                        } else {
                            // Unknown type, set defaults
                            item.packaging = item.unit || 'Botol';
                            item.unit = 'Mililiter';
                        }
                        updated = true;
                    }
                ...
                if (updated && inventory.length > 0) {
                    saveData();
                    localStorage.setItem('unitToPackagingMigration', 'done');
                    console.log('Migrated old units to packaging system');
                    alert('Data telah diupdate: Satuan lama menjadi Bungkusan, dan ditambahkan kolom Satuan baru');
                }
            }
            ...
            if (data.inventory && Array.isArray(data.inventory)) {
                ...
                if (confirm(`Impor ${itemCount} item, ${transactionCount} transaksi, dan ${warehouseCount} gudang? Data saat ini akan ditimpa.`)) {
                    ...
                    if (needsMigration) {
                        ...
                        alert('Data lama telah dimigrasi ke sistem baru (Bungkusan & Satuan terpisah)');
                    }
...
function clearAllData() {
    if (confirm('PERINGATAN: Semua data akan dihapus permanen! Yakin ingin melanjutkan?')) {
        if (confirm('Konfirmasi sekali lagi: Hapus SEMUA data stok, transaksi, dan gudang?')) {
            ...
            localStorage.removeItem('unitUpdateToMililiter'); // Clear old update flag
            localStorage.removeItem('unitToPackagingMigration'); // Clear migration flag
            updateDisplay();
...
function clearForm() {
    ...
    document.getElementById('itemPackaging').selectedIndex = 0;
...
function addItem() {
    ...
    if (!name || !packaging || !unit || !warehouseId) {
        alert('Nama barang, bungkusan, satuan, dan gudang harus diisi!');
        return;
    }
...
function updateStock() {
    ...
    if (item) {
        ...
        alert(`Stok berhasil diupdate!\n${item.name}: ${oldStock} → ${item.stock} ${item.packaging || 'pcs'}`);
    }
}
...
function moveToWarehouse() {
    ...
    updateDisplay();
    clearMoveForm();
    alert(`${quantity} ${item.packaging || 'pcs'} ${item.name} berhasil dipindah dari ${oldWarehouseName} ke ${newWarehouse.name}!`);
}
...
function editItem(id) {
    ...
    document.getElementById('editItemPackaging').value = item.packaging || 'Botol';
    document.getElementById('editItemUnit').value = item.unit || 'Mililiter';
...
function saveEditItem() {
    ...
    const newPackaging = document.getElementById('editItemPackaging').value;
    ...
    if (!newName || !newPackaging || !newUnit) {
        alert('Nama barang, bungkusan, dan satuan harus diisi!');
        return;
    }
...
function updateItemSelects() {
    ...
    sortedInventory.forEach(item => {
        ...
        const displayText = `${item.name} (${item.stock} ${item.packaging || 'pcs'}) - ${item.warehouseName}`;
    ...
    select.onchange = function() {
        ...
        warehouses.forEach(warehouse => {
            if (warehouse.id != selectedItem.warehouseId) { // Exclude current warehouse
                ...
                if (existingInTarget) {
                    option.textContent = `${warehouse.name} (Ada ${existingInTarget.stock} ${existingInTarget.packaging || 'pcs'})`;
                } else {
                    option.textContent = warehouse.name;
                }
